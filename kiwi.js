...
(function() {
  ...
  function extractStructArguments(definitions, prefix, fields, allowArrays) {
    ...
    for (var i = 0; i < fields.length; i++) {
      ...
      var arg = argumentForField(definitions, field.type, name);
      if (arg !== null) {
        args.push(arg);
        continue;
      }
      ...
      var typeArgs = extractStructArguments(definitions, name + '_', type.fields, false);
      if (typeArgs === null) {
        return null;
      }
  ...
  function emitReadField(cpp, definitions, definition, field, indent) {
    ...
    var args = extractStructArguments(definitions, '', [field], true);
    if (args !== null) {
  ...
  function compileSchemaCallbackCPP(schema) {
    ...
    for (var pass = 0; pass < 2; pass++) {
      ...
      for (var i = 0; i < schema.definitions.length; i++) {
        ...
        if (definition.kind === 'STRUCT') {
          var args = extractStructArguments(definitions, '', definition.fields, false);
          if (args !== null) {
  ...
  kiwi.compileSchemaCallbackCPP = compileSchemaCallbackCPP;
}());
